...
## FreeCADで見る（確実な方法）
FreeCAD 1.0 の **CSGインポータはOCCTブーリアン失敗（`Null input shape`）で不安定**なので、
**ネイティブ文書 `.FCStd` を用意しました**（Part APIで生成し、freecadcmdで検証済み）:

- **`torso_v3.FCStd`** を **ファイル > 開く**（またはダブルクリック）
  → 12個のソリッド（シェル＋RDK＋バッテリー＋SG90×2＋十字ホーン＋スピーカー＋サーボ台）が
     **編集可能ソリッド**で開きます。CSGインポータを通さないのでエラーになりません。
- 色付き・自動フィットで見たいときは **マクロ > マクロ… > `torso_v3_freecad.py` > 実行**。
- （STL版 `stl/*.stl` も同フォルダにあり、こちらも確実に開けます。）

> `.csg` はFreeCADのバグで開けないため非推奨。`.FCStd` かマクロを使ってください。
