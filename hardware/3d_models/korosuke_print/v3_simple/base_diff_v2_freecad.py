"""
コロ助 差動2輪ドライブベースv2(精密化: バルクヘッドネジ止め/キャスタ長穴) — FreeCADネイティブ組立(CSGインポータ不使用=確実に開ける)。

  A) GUI: マクロ > マクロ… > このファイル > 実行  (色付き・自動フィット)
  B) CLI: "C:\\Program Files\\FreeCAD 1.0\\bin\\freecadcmd.exe" base_diff_v2_freecad.py
          → base_diff_v2.FCStd に保存(ダブルクリックで開ける)
...
def build():
    doc = App.newDocument("KorosukeDiffBaseV2")
    # 印刷ベース = STLメッシュ
    mesh(doc, "DriveBase", "stl/base_diff_v2.stl", (0.85, 0.46, 0.02))
...
if HAS_GUI:
    ...
    out = os.path.join(HERE, "base_diff_v2.FCStd"); doc.saveAs(out)
