...
settings = {
    ...
    "oj_fm": 5,            # 声の高さ(Open JTalk -fm)。既定5 (旧既定: 9→8→5)
...
<section id="view-monitor" class="view">
  <div class="card full" style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap">
    <button id="kact" class="tab active" style="font-size:1.05rem" onclick="toggleActive()">🟢 Korosuke Active</button>
    <small style="color:var(--mut)">Inactive = 挨拶・話しかけ反応・LLM会話・腕を一括停止(発表の待機用)。Settingsの「反応と会話」チェックと連動</small>
  </div>
...
let kActive=true;
function toggleActive(){ applyActive(!kActive); }
function applyActive(on){
  kActive=on;
  ['react_greet','react_speech','use_llm','use_arm'].forEach(k=>set(k,on?1:0));
  [['c_g',1],['c_s',1],['c_llm',1],['c_arm',1]].forEach(([id])=>{
    const el=document.getElementById(id); if(el) el.checked=on; });
  const b=document.getElementById('kact');
  b.textContent=on?'🟢 Korosuke Active':'🔴 Korosuke Inactive';
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/voices"):      # 利用可能な声モデル(.htsvoice)一覧
            ...
            self.wfile.write(body)
            return
        if self.path.startswith("/disptext"):    # 胴体ディスプレイ用: 認識テキスト+返答(display_send.pyが取得)
            with lock:
                fin = state.get("finals") or []
                heard = fin[0] if fin else ""
                reply = state.get("speech") or ""
            if heard[:2].isdigit() and " " in heard:  # "HH:MM:SS text" → text
                heard = heard.split(" ", 1)[1]
            body = json.dumps({"heard": heard, "reply": reply}, ensure_ascii=False).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self.send_header("Content-Length", str(len(body)))
            self.end_headers()
