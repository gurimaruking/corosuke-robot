...
| 要件 | 実装 |
|---|---|
| SG90×2 でロープアームを引く（ネジ止め） | **腕穴直下のシェル一体コの字タワー**（実機レイアウト準拠・2026-07-28変更）。上開放スロットに**上から落とし込み→前面から付属M2×2**。ロープは腕穴→天面Vノッチ→ホーンへほぼ一直線。**軸側の端を胴中心側に向ける**（左右で逆）。旧デッキ吊りブラケット(SHOW=4)はオプション |
