...
**共有バス配線(現在の採用)**: SCLK/MOSI/DC/RST を2枚へY分岐、CSのみ個別。
IO4/5/6(SCLK/MOSI/DC)は**P1の連続3パッド**なので、市販の**サーボ用Yハーネス
(2.54mm 3ピン 1→2分岐)1本**でまとめて分岐できる。RSTはもう1本のY(または1ピンY)。
...
| P1-4 | IO17 | 空き | | P2-4 | IO47 | 空き |
| P1-5 | IO16 | **目CS右** | | P2-5 | IO48 | 空き |
| P1-6 | IO15 | **目CS左** | | P2-6 | IO45 | ⚠使用禁止(strapping) |
| P1-7 | IO7 | **目RST(共有Y分岐)** | | P2-7 | IO38 | 空き |
| P1-8 | IO6 | **目DC(共有Y分岐)** | | P2-8 | IO39 | 空き |
| P1-9 | IO5 | **目MOSI(共有Y分岐)** | | P2-9 | IO40 | 空き |
| P1-10 | IO4 | **目SCLK(共有Y分岐)** | | P2-10 | IO41 | **サーボ左** |
| P1-11 | 3V3 | 目VCC(2枚とも) | | P2-11 | IO42 | **サーボ右** |
| P1-12 | GND | 目GND | | P2-12 | IO2 | 撫でタッチ(T2, 任意) |

- **P1列だけで目の配線が電源込みで完結**。CSだけ左右別(IO15/IO16)、他はY分岐で共通。
- **独立配線(Y分岐なし・点対点)にも切替可能**: platformio.iniの`EYER_*`を
  IO38/39/48/47/21(P2列)へ変えて焼き直すだけ。SCLK/MOSIはGPIOマトリクスの
  「1出力信号→複数パッド複製」(`mirrorSpi3Bus()`)で左右両ピンに同時出力される。
