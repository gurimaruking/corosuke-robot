...
> **Status:** draft v0.1 (2026-06-25). Builds directly on the Stage 1 result
...
### 3.3 Roadmap (now 2026-06-25 → hard deadline 2026-07-15; rebate target 2026-07-08)

| Week | Milestone | Exit criterion |
|------|-----------|----------------|
| **6/25–6/30** | ROS 2 on RDK X5; `korosuke_msgs`; `serial_bridge` ↔ one ESP32; eye coprocessor bring-up (GC9A01×2 + RoboEyes) | a ROS 2 topic moves a servo; eyes blink on command |
| **7/1–7/7** | `vision_node` (BPU) → `/face_pose`; gaze (eyes+neck) tracking; on-device dialogue loop (LLM+VOICEVOX) + lipsync; `expression_node` | Korosuke looks at a face and says a "…nari" line with lipsync |
| **7/8 (MVP / rebate gate)** | Integrated demo: see → turn → greet → emote → gesture, fully on-device | G1–G6 met; **competition-critical line complete** |
| **7/9–7/15** | Polish; 3–7 min demo video + benchmark table; bipedal stretch *iff* QDD revived | Stage 3 submission package |
